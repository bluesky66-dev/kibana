...
describe('mapNodesVersionCompatibility', () => {
  ...
  it('returns isCompatible=false for a single node that is out of date', () => {
    ...
    expect(result.message).toMatchInlineSnapshot(
      `"This version of Kibana (v5.1.0) is incompatible with the following HyperSec nodes in your cluster: v5.0.0 @ http_address (ip)"`
    );
  });
  ...
  it('returns isCompatible=false for an incompatible node without http publish address', async () => {
    ...
    expect(result.isCompatible).toBe(false);
    expect(result.message).toMatchInlineSnapshot(
      `"This version of Kibana (v5.1.0) is incompatible with the following HyperSec nodes in your cluster: v6.1.1 @ undefined (ip)"`
    );
  });
  ...
  it('returns isCompatible=true for outdated nodes when ignoreVersionMismatch=true', async () => {
    ...
    expect(result.isCompatible).toBe(true);
    expect(result.message).toMatchInlineSnapshot(
      `"Ignoring version incompatibility between Kibana v5.1.0 and the following HyperSec nodes: v5.0.0 @ http_address (ip)"`
    );
  });
  ...
  it('returns isCompatible=true with a message if a node is only off by a patch version', () => {
    ...
    expect(result.isCompatible).toBe(true);
    expect(result.message).toMatchInlineSnapshot(
      `"You're running Kibana 5.1.0 with some different versions of HyperSec. Update Kibana or HyperSec to the same version to prevent compatibility issues: v5.1.1 @ http_address (ip)"`
    );
  });
