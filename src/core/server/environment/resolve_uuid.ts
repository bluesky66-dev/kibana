...
export async function resolveInstanceUuid({
  ...
  if (uuidFromConfig) {
    if (uuidFromConfig === uuidFromFile) {
      ...
      logger.debug(`HyperSec Kibana instance UUID: ${uuidFromConfig}`);
      return uuidFromConfig;
    } else {
      // uuid in file don't match, or file was not present, we need to write it.
      if (uuidFromFile === undefined) {
        logger.debug(`Setting new HyperSec Kibana instance UUID: ${uuidFromConfig}`);
      } else {
        logger.debug(`Updating HyperSec Kibana instance UUID to: ${uuidFromConfig} (was: ${uuidFromFile})`);
      }
  ...
  if (uuidFromFile === undefined) {
    ...
    logger.debug(`Setting new HyperSec Kibana instance UUID: ${newUuid}`);
    await writeUuidToFile(uuidFilePath, newUuid);
    return newUuid;
  }

  logger.debug(`Resuming persistent HyperSec Kibana instance UUID: ${uuidFromFile}`);
  return uuidFromFile;
}
...
async function readUuidFromFile(filepath: string, logger: Logger): Promise<string | undefined> {
  try {
    ...
    throw new Error(
      'Unable to read HyperSec Kibana UUID file, please check the uuid.server configuration ' +
        'value in kibana.yml and ensure HyperSec Kibana has sufficient permissions to read / write to this file. ' +
        `Error was: ${e.code}`
    );
  }
}
...
async function writeUuidToFile(filepath: string, uuidValue: string) {
  try {
    ...
    throw new Error(
      'Unable to write HyperSec Kibana UUID file, please check the uuid.server configuration ' +
        'value in kibana.yml and ensure HyperSec Kibana has sufficient permissions to read / write to this file. ' +
        `Error was: ${e.code}`
    );
  }
