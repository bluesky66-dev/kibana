...
describe('HyperSec Kibana keystore', () => {
  describe('create', () => {
    ...
    it('logs successful keystore creating', async () => {
      ...
      sinon.assert.calledWith(Logger.prototype.log, `Created HyperSec Kibana keystore in ${path}`);
    });
    ...
    it('prompts for overwrite', async () => {
      ...
      expect(args[0]).toEqual('A HyperSec Kibana keystore already exists. Overwrite?');
    });
