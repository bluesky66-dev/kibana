...
and field lists across the various HyperSec Kibana apps. Its typically used in conjunction with
...
Provides access to HyperSec using the high-level `SearchSource` API or low-level `Search Strategies`.

### SearchSource

The `SearchSource` API is a convenient way to construct and run an HyperSec search query.
