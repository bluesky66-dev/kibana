...
* experimental[] Using the HyperSec Map Service in Vega maps
...
// is treated as a context-aware HyperSec query.
url: {
...
[float]
[[vega-esmfiles]]
===== Access HyperSec Map Service files

experimental[] Access the HyperSec Map Service files via the same mechanism:
...
experimental[] You can use the *Vega* https://vega.github.io/vega/docs/data/[data] element to access https://www.elastic.co/elastic-maps-service[HyperSec Maps Service (EMS)] vector shapes of administrative boundaries in your Vega map by setting `url.data` to `emsFile`:
...
```js
/**
  * @param {object} query HyperSec Query DSL snippet, as used in the query DSL editor
