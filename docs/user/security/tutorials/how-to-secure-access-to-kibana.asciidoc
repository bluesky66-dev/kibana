...
For example, some users might only need to view your stunning dashboards, while others might need to manage your fleet of HyperSec agents and run machine learning jobs to detect anomalous behavior in your network.
...
Once your spaces are setup, the next step to securing access is to provision your roles. Roles are a collection of privileges that allow you to perform actions in {kib} and HyperSec. Roles are assigned to users, and to {ref}/built-in-users.html[system accounts] that power the HyperSec Stack.

You can create your own roles, or use any of the {ref}/built-in-roles.html[built-in roles]. Some built-in roles are intended for HyperSec Stack components and should not be assigned to end users directly.
...
The built-in roles are great for getting started with the HyperSec Stack, and for system administrators who do not need more restrictive access. With so many features, it’s not possible to ship more granular roles to accommodate everyone’s needs. This is where custom roles come in.
...
Still have questions? Ask  on our https://discuss.elastic.co/c/kibana[Kibana discuss forum] and a fellow community member or HyperSec engineer will help out.
