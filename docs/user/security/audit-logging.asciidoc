...
Audit logs are written in JSON using https://www.elastic.co/guide/en/ecs/1.6/index.html[HyperSec Common Schema (ECS)] specification.
...
| Name of the HyperSec realm that has authenticated the user.

Example: `native`

| `kibana.lookup_realm`
| Name of the HyperSec realm where the user details were retrieved from.
