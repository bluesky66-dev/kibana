...
NOTE: The {kib} Console supports only HyperSec APIs. You are unable to interact with the {kib} APIs with the Console and must use `curl` or another HTTP tool instead. For more information, refer to <<console-kibana,Console>>.
...
To use token-based authentication, you use the same username and password that you use to log into HyperSec.  
...
To use key-based authentication, you create an API key using the HyperSec Console, then specify the key in the header of your API calls.
