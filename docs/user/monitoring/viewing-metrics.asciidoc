...
. If the HyperSec {security-features} are enabled on the monitoring cluster, you
...
. If the HyperSec {security-features} are enabled on the {kib} server, only users
...
If the HyperSec {security-features} are enabled, log in.
--
