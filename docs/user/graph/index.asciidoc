...
HyperSec index are related. You can explore the connections between
...
box with existing HyperSec indices--you don't need to store any
...
about the documents and terms in your HyperSec index. A _graph_ is really
...
aren't graph geeks and avoid any confusion with the nodes in an HyperSec
cluster.

The graph vertices are simply the terms that you've already indexed. The
connections are derived on the fly using HyperSec aggregations. To
identify the most _meaningful_ connections, the graph API leverages
HyperSec relevance scoring. The same data structures and relevance ranking
tools built into HyperSec to support text searches enable the graph API to
...
But what about performance, you ask? The HyperSec aggregation framework
...
multi-node clusters and scales with your HyperSec deployment.
