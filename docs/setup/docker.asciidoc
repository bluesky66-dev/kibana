...
Docker images for Kibana are available from the HyperSec Docker registry. The
...
against the HyperSec Docker registry.
...
Kibana can be quickly started and connected to a local HyperSec container for development
