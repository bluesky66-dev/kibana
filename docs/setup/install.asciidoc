...
If you are running our hosted HyperSec Service on HyperSec Cloud, you access Kibana with a single click. (You can {ess-trial}[sign up for a free trial] and start exploring data in minutes.)
...
systems.  Debian packages may be downloaded from the HyperSec website or from
...
HyperSec website or from our RPM repository.
+
<<rpm>>
...
downloaded from the HyperSec Docker Registry.
+
<<docker,Running Kibana on Docker>>

`brew`::

Formulae are available from the HyperSec Homebrew tap for installing {kib} on macOS with the Homebrew package manager.
+
<<brew>>

IMPORTANT: If your HyperSec installation is protected by
