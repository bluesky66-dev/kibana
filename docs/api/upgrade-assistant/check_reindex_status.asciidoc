...
<4> Task ID of the reindex task in HyperSec. Only present if reindexing has started.
<5> Percentage of how far the reindexing task in HyperSec has progressed, in decimal from from 0 to 1.
...
`40`::
  The reindex task in HyperSec has started.

`50`::
  The reindex task in HyperSec has completed.
...
`2`::
  Specifies to convert documents to support HyperSec Common Schema. Only applies to APM indices created in 6.x.
