...
HyperSec Maps and the region map and coordinate map.visualizations.
...
If the plugin cannot retrieve a license from **HyperSec**, it will emit `an empty license` object.
...
and does not communicate with HyperSec directly.
