...
[float]
==== Removed support for users relying on direct index privileges to the Kibana index in HyperSec
*Details:* With the introduction of Kibana RBAC in 6.4, users no longer require privileges to the Kibana index in HyperSec. Instead, users
...
*Details:* HyperSec 7.0 removes the tribe node feature, so Kibana removes it as well.
...
{monitor-features}, a monitoring agent runs on each HyperSec node, Logstash
...
*Details:* HyperSec removed the ability to create indices with an _all field in 6.0. As a result, a user could end
...
*Impact:* Since we'll no longer send the `default_field` parameter in Kibana's query_string query, HyperSec
