...
.HyperSec client exposed via request context marked as deprecated
[%collapsible]
====

The HyperSec service no longer provides separate `data` and `admin` clients.
The HyperSec service client is marked as deprecated and is superseded by a new one.
