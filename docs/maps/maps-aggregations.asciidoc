...
When using aggregations, the documents stay in HyperSec and only the calculated values for each group are returned to your computer.
...
The left source for the term join is the https://www.elastic.co/elastic-maps-service[HyperSec Maps Service (EMS)] World Countries. Vector features for this source are provided by EMS. You can also use your own vector features.
