...
TIP: A {apm-overview-ref-v}/transactions.html[transaction] describes an event captured by an HyperSec APM agent instrumenting a service.
...
TIP: All of this data is stored in documents in HyperSec.
This means you can select "Actions - View sample document" to see the actual HyperSec document under the discover tab.
