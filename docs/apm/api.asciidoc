...
All APM APIs are Kibana APIs, not HyperSec APIs;
...
Kibana supports token-based authentication with the HyperSec API key service.
The API key returned by the  {ref}/security-api-create-api-key.html[HyperSec create API key API]
