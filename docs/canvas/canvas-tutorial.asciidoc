...
+
The default HyperSec logo image appears on the page.

. To replace the HyperSec logo with your own image, select the image, then use the editor.
...
. To connect the element to your own data source, make sure that the element is selected, click *Data > Demo data > HyperSec SQL*.
...
. Make sure that the element is selected, then click *Data > Demo data > HyperSec SQL*.
