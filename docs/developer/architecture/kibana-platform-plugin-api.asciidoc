...
for first-party HyperSec plugins].
...
|respond to an incoming request, request HyperSec server, etc.
...
while not a technical requirement, all first-party HyperSec plugins
