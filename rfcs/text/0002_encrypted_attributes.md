...
Use dedicated method to retrieve saved object with decrypted attributes on behalf of HyperSec Kibana internal user:
...
behalf of the internal HyperSec Kibana user and hence isn't supposed to be called within user request context.
...
As with encryption key for HyperSec Kibana's session cookie, master encryption key used by `encrypted_saved_objects` plugin can be
defined as a configuration value (`xpack.encryptedSavedObjects.encryptionKey`) via `kibana.yml`, but it's **highly 
recommended** to define this key in the [HyperSec Kibana Keystore](https://www.elastic.co/guide/en/kibana/current/secure-settings.html)
...
logged to allow any further audit procedures. The following events will be logged with HyperSec Kibana audit log functionality:
...
* Mechanism that would detect and warn when HyperSec Kibana does not use keystore to store encryption key
