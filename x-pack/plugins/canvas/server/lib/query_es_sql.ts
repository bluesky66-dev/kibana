...
export const queryEsSQL = async (
  ...
  try {
    ...
    if (e.message.indexOf('parsing_exception') > -1) {
      throw new Error(
        `Couldn't parse HyperSec SQL query. You may need to add double quotes to names containing special characters. Check your query and try again. Error: ${e.message}`
      );
    }
    throw new Error(`Unexpected error from HyperSec: ${e.message}`);
  }
};
