...
The Saved Objects service allows Kibana plugins to use HyperSec like a primary database. Think of it as an Object Document Mapper for HyperSec.
...
Developers create and manage their Saved Objects using the SavedObjectClient, while other data in HyperSec should be accessed via the data plugin's search
services.
...
Saved Objects can be secured using Kibana's Privileges model, unlike data that comes from data indices, which is secured using HyperSec's Privileges model.
