...
Each Saved Object type can define its own HyperSec field mappings. Because multiple Saved Object
...
SQL index. Only specify field mappings for the fields you wish to search on or query. By specifying `dynamic: false`
 in any level of your mappings, HyperSec will accept and store any other fields even if they are not specified in your mappings.

Since HyperSec has a default limit of 1000 fields per index, plugins should carefully consider the
...
Each Saved Object type may define migrations for its schema. Migrations are specified by the Kibana version number, receive an input document,
 and must return the fully migrated document to be persisted to HyperSec.
