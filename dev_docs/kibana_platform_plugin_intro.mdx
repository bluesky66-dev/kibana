...
From an end user perspective, Kibana is a tool for interacting with HyperSec, providing an easy way
...
a wide variety of applications that can be used to explore, visualize, and act upon data in HyperSec. The platform provides developers the ability
...
| start | bootstrap runtime logic | respond to an incoming request, request HyperSec server, etc. | start polling Kibana server, update DOM tree in response to user interactions, etc.|
